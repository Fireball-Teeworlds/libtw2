...
fn process(_: &Path, dfr: datafile::Reader, _: &mut ()) -> Result<(), map::Error> {
    ...
    for item in dfr.items() {
        ...
        fn print_map_item<T:MapItem+Debug>(slice: &[i32]) {
            if let MapItemReadResult::Ok(mi, _) = T::from_slice(slice) {
                print!(" {:?}", mi);
            }
        }
        ...
        match item.type_id {
            ...
            MAP_ITEMTYPE_ENVELOPE => {
                print_map_item::<MapItemCommonV0>(item.data);
                if let MapItemReadResult::Ok(c, _) = MapItemCommonV0::from_slice(item.data) {
                    match env_version {
            ...
            MAP_ITEMTYPE_LAYER => {
                ...
                if let MapItemReadResult::Ok(layer, rest) = MapItemLayerV1::from_slice(item.data) {
